...
st.write(f'The most efficient policies to save lives are the following: {pol_dict.values()}. Slide to select policy intensity, and simulate the efficiency of your policy')
...
i1_min, i1_max=(int(policies_df[policies_df['PolicyType']==pol[0]].PolicyValue.min()),
    int(policies_df[policies_df['PolicyType']==pol[0]].PolicyValue.max()))
i1 = st.slider('H6', i1_min, i1_max, int(Slide1_curr))
...
i2 = st.slider('H4', i2_min, i2_max, int(Slide2_curr))
...
i3 = st.slider('H2', i3_min, i3_max, int(Slide3_curr))
...
i4 = st.slider('E3', i4_min, i4_max, int(Slide4_curr))
...
i5 = st.slider('C2', i5_min, i5_max, int(Slide5_curr))
...
X_template=pd.read_csv('/Users/paulcharpentier/Desktop/Coronanet_files/X_input.csv')
...
if saved_lives < 0 :
    ...
    if filt == 'Cases':
        s2=f'Up to **{int(saved_lives)}** lives can be saved with the policies you selcted'
    else:
